...
browser.a(class: 'login-popup').when_present(timeout).click # wait till page is loaded, then click the link with the "login-popup" class
browser.form(id: 'packt-user-login-form').text_field(name: 'email').set(ENV['PACKTPUB_EMAIL']) # enter email in form
browser.form(id: 'packt-user-login-form').text_field(name: 'password').set(ENV['PACKTPUB_PASSWORD']) # enter password in form
browser.form(id: 'packt-user-login-form').button(type: 'submit').click # click login button
browser.div(id: 'account-bar-logged-in').wait_until_present(timeout) # user is logged in when a div with this id appears
...
if ENV['SEND_NOTIFICATION'] == 'true'
  message = "Added new free book to your library: #{book_title}, #{ebook_url}\n\n
Download:\n
 - PDF : https://www.packtpub.com/ebook_download/#{ebook_id}/pdf\n
 - Epub: https://www.packtpub.com/ebook_download/#{ebook_id}/epub\n
 - Mobi: https://www.packtpub.com/ebook_download/#{ebook_id}/mobi\n
\n\n
This email was generated by https://github.com/cars10/packtpub-script"
  Pony.mail(
